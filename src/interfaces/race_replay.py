...
from src.services.voice_radio import RadioVoice
...
class F1RaceReplayWindow(arcade.Window):
    def __init__(self, frames, track_statuses, example_lap, drivers, title,
        ...
        self.controls_popup_comp.set_size(340, 275) # width/height of the popup box
        ...
        # Embedded AI Narrator state
        self.event_detector = RaceEventDetector()
        self.granite = GraniteClient()
        self.radio_voice = RadioVoice()
        self.ai_enabled = True
        self.ai_commentary = []
        self.ai_pending_commentary = None
    ...
    def on_draw(self):
        ...
        # Draw AI Commentary Box
        if self.visible_hud:
            self.draw_ai_commentary_box()
    ...
    def on_update(self, delta_time: float):
        ...
        # Update embedded AI narrator
        self._update_ai_narrator()
    ...
    def on_mouse_press(self, x: float, y: float, button: int, modifiers: int):
        # Check click on AI Commentary Box toggle button
        if self.visible_hud and self._check_ai_toggle_click(x, y):
            return
    ...
    def _update_ai_narrator(self):
        if not self.ai_enabled or not self.frames:
            return
        ...
        # Build payload for event detector
        payload = {
            "frame": current_frame,
            "session_data": {
                "lap": current_frame["drivers"][next(iter(current_frame["drivers"]))].get("lap", 1) if current_frame["drivers"] else 1,
                "total_laps": self.total_laps,
                "time": "00:00:00"
            },
            "track_status": current_track_status
        ...
        # Safe extraction of time_str
        hours = int(current_time // 3600)
        minutes = int((current_time % 3600) // 60)
        seconds = int(current_time % 60)
        time_str = f"{hours:02}:{minutes:02}:{seconds:02}"
        payload["session_data"]["time"] = time_str

        # Process the frame
        events = self.event_detector.process_frame(payload)
        for event in events:
            # If we've already got too many pending, skip
            if self.ai_pending_commentary is not None:
                break

            self.ai_pending_commentary = event

            # Request async commentary
            prompt = event["prompt"]
            ...
            def _callback(result, ev=event_copy):
                ev["narration"] = result
                # Add to history
                self.ai_commentary.append(ev)
                if len(self.ai_commentary) > 3:
                    self.ai_commentary.pop(0)
                
                # Speak it!
                if self.ai_enabled:
                    self.radio_voice.speak(result)

                # Clear pending status
                if self.ai_pending_commentary and self.ai_pending_commentary.get("prompt") == ev.get("prompt"):
                    self.ai_pending_commentary = None

            self.granite.generate_async(prompt, _callback)

    def draw_ai_commentary_box(self):
        # 1. Geometry
        box_width = 600
        box_height = 125
        center_x = (self.left_ui_margin + self.width - self.right_ui_margin) / 2
        center_y = 202  # Sits right above controls/progress bar Y space
        ...
        # 2. Draw Translucent Card
        arcade.draw_rect_filled(arcade.XYWH(center_x, center_y, box_width, box_height), (15, 15, 25, 220))

        # F1 Red border accent on the left
        border_color = (225, 6, 0)  # F1 Red
        if not self.granite.is_available:
            border_color = (255, 140, 0)  # Orange for warning

        arcade.draw_rect_outline(arcade.XYWH(center_x, center_y, box_width, box_height), (60, 60, 80, 255), 1)
        arcade.draw_rect_filled(arcade.XYWH(left + 2, center_y, 4, box_height), border_color)

        # 3. Draw Header
        header_text = "📻 AI TEAM RADIO"
        if not self.granite.is_available:
            header_text = "⚠️ AI TEAM RADIO — CREDENTIALS MISSING"
        ...
        # Shortcut Hint and Button
        if self.granite.is_available:
            # Shortcut Hint
            arcade.draw_text(
                "Press [A] to hide HUD box",
                right - 275,
                top - 18,
                (150, 150, 160),
                9,
            ...
            # Draw AI Pause Button (Left)
            btn1_center_x = right - 192.5
            btn1_center_y = top - 18
            btn1_width = 115
            btn1_height = 20

            btn1_bg = (30, 30, 45, 255) if self.ai_enabled else (46, 204, 113, 50)
            btn1_outline = (225, 6, 0, 255) if self.ai_enabled else (46, 204, 113, 255)
            btn1_text = "⏸ PAUSE AI" if self.ai_enabled else "▶ ENABLE AI"
            btn1_text_color = arcade.color.WHITE if self.ai_enabled else (46, 204, 113)

            arcade.draw_rect_filled(arcade.XYWH(btn1_center_x, btn1_center_y, btn1_width, btn1_height), btn1_bg)
            arcade.draw_rect_outline(arcade.XYWH(btn1_center_x, btn1_center_y, btn1_width, btn1_height), btn1_outline, 1)
            arcade.draw_text(
                btn1_text,
                btn1_center_x,
                btn1_center_y,
            ...
            # Draw Voice Toggle Button (Right)
            btn2_center_x = right - 72.5
            btn2_center_y = top - 18
            btn2_width = 115
            btn2_height = 20
            
            is_premium = self.radio_voice.use_elevenlabs
            btn2_bg = (235, 177, 52, 50) if is_premium else (30, 30, 45, 255)
            btn2_outline = (235, 177, 52, 255) if is_premium else (150, 150, 160, 255)
            btn2_text = "🎙️ ELEVENLABS" if is_premium else "🎙️ MAC VOICE"
            btn2_text_color = (255, 215, 0) if is_premium else (150, 150, 160)

            arcade.draw_rect_filled(arcade.XYWH(btn2_center_x, btn2_center_y, btn2_width, btn2_height), btn2_bg)
            arcade.draw_rect_outline(arcade.XYWH(btn2_center_x, btn2_center_y, btn2_width, btn2_height), btn2_outline, 1)
            arcade.draw_text(
                btn2_text,
                btn2_center_x,
                btn2_center_y,
            ...
            # Just show a standard hint if credentials are missing
            arcade.draw_text(
                "Press [A] to hide",
                right - 15,
                top - 18,
        ...
        if not self.ai_enabled:
            dot_color = (120, 120, 130)  # Grey for paused
        elif not self.granite.is_available:
            dot_color = (231, 76, 60)  # Red
        elif self.ai_pending_commentary is not None:
            # Pulsing amber dot
            import time
            pulse = 0.5 + 0.5 * np.sin(time.time() * 6.0)
            dot_color = (255, 140, 0, int(150 + 105 * pulse))
        else:
            dot_color = (46, 204, 113)  # Green for active

        arcade.draw_circle_filled(left + 130 if not self.granite.is_available else left + 120, top - 18, 4, dot_color)
        ...
        if not self.granite.is_available:
            ...
            content_text = "AI commentary is currently paused. Telemetry processing and API request updates are suspended. Click 'ENABLE AI' to resume."
            ...
            # Display latest narration
            latest = self.ai_commentary[-1]
            content_text = latest.get("narration", "Analyzing telemetry...")
            text_color = arcade.color.WHITE
            is_italic = False
        else:
            content_text = "Waiting for race events to commentate..."
            text_color = (150, 150, 160)
            is_italic = True
        ...
        arcade.draw_text(
            ...
            top - 35,
            text_color,
            10 if not self.granite.is_available else 11,
            multiline=True,
            width=box_width - 30,
            italic=is_italic,
            anchor_y="top"
    ...
    def _check_ai_toggle_click(self, x: float, y: float) -> bool:
        """Check if a mouse click hit the AI enable/pause button."""
        if not self.granite.is_available:
            return False

        # Calculate exact button coordinates
        box_width = 600
        center_x = (self.left_ui_margin + self.width - self.right_ui_margin) / 2
        right = center_x + box_width / 2
        ...
        # Check AI Pause Button (Left)
        btn1_center_x = right - 192.5
        btn1_center_y = top - 18
        btn1_width = 115
        btn1_height = 20

        l1, r1 = btn1_center_x - btn1_width / 2, btn1_center_x + btn1_width / 2
        b1, t1 = btn1_center_y - btn1_height / 2, btn1_center_y + btn1_height / 2

        if l1 <= x <= r1 and b1 <= y <= t1:
            self.ai_enabled = not self.ai_enabled
            return True

        # Check Voice Toggle Button (Right)
        btn2_center_x = right - 72.5
        btn2_center_y = top - 18
        btn2_width = 115
        btn2_height = 20
        
        l2, r2 = btn2_center_x - btn2_width / 2, btn2_center_x + btn2_width / 2
        b2, t2 = btn2_center_y - btn2_height / 2, btn2_center_y + btn2_height / 2

        if l2 <= x <= r2 and b2 <= y <= t2:
            self.radio_voice.toggle_elevenlabs()
            return True
